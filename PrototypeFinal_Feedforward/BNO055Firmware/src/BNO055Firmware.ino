...
void feedforward(uint8_t axis, int64_t freq_in)
{
    ...
    //get angular velocity (radians per sec) in x, y, z
    float xin = gyro.x()*TWO_PI;
    float yin = gyro.y()*TWO_PI;
    float zin = gyro.z()*TWO_PI;
    ...
    //motor should turn in same direction as pidErr
    if (pidErr < 0){
        freq_in = -freq_in;
    }
    ...
    double Kp = 1;
    double adjustedSpeed = Kp*freq_in;
    //double adjustedSpeed = Kp*pidErr/TWO_PI;

    //set motor output speed
    motorB.setSpeed(adjustedSpeed);
}
